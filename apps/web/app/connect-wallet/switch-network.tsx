  ...
  return (
    <>
      <button onClick={() => (document.getElementById('switch_network_modal') as any).showModal()}>
        {isThanos ? <ThanosCircleSymbol networkType='testnet' /> :
      ...
      <dialog id="switch_network_modal" className="modal bg-black bg-opacity-75">
        <div className="modal-box w-[360px] bg-black ring-1 ring-[#232429]">
          <div className='flex justify-between items-center mb-[21px]'>
            <h3 className="font-semibold text-xl">Switch Networks</h3>
            <form method="dialog">
          ...
          <div className='flex flex-col gap-[9px]'>
            <NetworkSwitchBtn networkType='mainnet' isDisabled={true}>
              <p className='font-semibold'>
                Thanos mainnet <span className='text-trh-gray font-bold'>Coming Soon</span>
              </p>
            </NetworkSwitchBtn>
            <NetworkSwitchBtn onClick={() => handleSwitchNetwork(thanosSepolia.id)} networkType='testnet' isDisabled={false}>
              <p className='font-semibold'>
